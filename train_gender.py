"""
Gender classification — MFCC + 1D CNN
Classes : female / male
Dataset : audio_dataset/females/*.wav
          audio_dataset/males/*.wav
"""
...
CLASS_MAP    = {"females": "female", "males": "male"}
MAX_LEN      = 200
EPOCHS       = 100
BATCH        = 32
CACHE_X      = "cache/gender_X.npy"
CACHE_Y      = "cache/gender_y.npy"
...
# Plots
def save_plots(history, y_true, y_pred, classes, prefix="plots/gender"):
    os.makedirs(os.path.dirname(prefix), exist_ok=True)
    cm = confusion_matrix(y_true, y_pred)

    fig, axes = plt.subplots(2, 2, figsize=(14, 10))
    fig.suptitle("Gender Classification", fontsize=14)
    ...
    for i, cls in enumerate(classes):
        ...
        axes[1, 1].bar(x + (i - len(classes)/2 + .5) * w, vals, w, label=cls)
        for j, v in enumerate(vals):
            axes[1, 1].text(j + (i - len(classes)/2 + .5) * w, v + .02,
...
if __name__ == "__main__":
    ...
    print("Model saved -> models/gender_model.h5")
